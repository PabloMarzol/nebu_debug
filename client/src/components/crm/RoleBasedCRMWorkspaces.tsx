...
interface WorkspaceData {
  ...
  requiredAccess: "full" | "read" | "limited";
...
export function RoleBasedCRMWorkspaces() {
  const { 
    userRole, 
    permissions, 
    hasWorkspaceAccess, 
  ...
  // Filter workspaces based on user role and permissions
  const accessibleWorkspaces = workspaces.filter(workspace => {
    // Check if user has access to this workspace
    if (!hasWorkspaceAccess(workspace.id, workspace.requiredAccess)) return false;
    
    // Hide internal-only workspaces from client users
    if (workspace.internalOnly && isClientUser()) return false;
  ...
  if (!activeWorkspace && accessibleWorkspaces.length > 0) {
    setActiveWorkspace(accessibleWorkspaces[0].id);
  }
  ...
  const handleWorkspaceChange = (workspaceId: string) => {
    ...
    if (workspace && hasWorkspaceAccess(workspaceId, workspace.requiredAccess)) {
  ...
  const getAccessLevelIcon = (workspaceId: string) => {
    const workspacePermission = permissions.workspaces.find(w => w.workspace === workspaceId);
    if (!workspacePermission) return <Lock className="w-4 h-4 text-red-400" />;
    
    switch (workspacePermission.access) {
      case "full": return <CheckCircle className="w-4 h-4 text-green-400" />;
      case "read": return <Eye className="w-4 h-4 text-blue-400" />;
      case "limited": return <EyeOff className="w-4 h-4 text-yellow-400" />;
      default: return <Lock className="w-4 h-4 text-red-400" />;
    }
  };
  ...
  const renderWorkspaceContent = () => {
    ...
    if (!hasWorkspaceAccess(currentWorkspace.id, currentWorkspace.requiredAccess)) {
      return (
        <Alert className="bg-red-900 border-red-600">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            You don't have permission to access this workspace. Contact your administrator for access.
          </AlertDescription>
        </Alert>
      );
    }
        ...
        <Card className="bg-gray-800 border-gray-700 max-w-md">
          ...
          <CardContent>
            <p className="text-gray-400 mb-4">
              Please log in to access the CRM workspaces.
            </p>
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                ...
                <Badge className="absolute -top-2 -right-2 bg-red-500 text-white text-xs px-1">
                  {accessibleWorkspaces.reduce((sum, w) => sum + w.notifications, 0)}
                </Badge>
              </div>
            </div>
          </div>
      ...
      {showAccessDenied && (
        <div className="px-6 py-4">
          <Alert className="bg-red-900 border-red-600">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>
              Access denied. You don't have permission to view this workspace.
            </AlertDescription>
          </Alert>
        </div>
      )}
      ...
      <div className="px-6 py-4 border-b border-gray-800">
        <div className="flex space-x-2 overflow-x-auto">
          {accessibleWorkspaces.map((workspace) => {
            ...
            const hasAccess = hasWorkspaceAccess(workspace.id, workspace.requiredAccess);
            ...
            return (
              ...
              >
                ...
                {getAccessLevelIcon(workspace.id)}
                {workspace.notifications > 0 && hasAccess && (
      ...
      {isClientUser() && (
        <div className="px-6 py-4 border-b border-gray-800">
          <Alert className="bg-blue-900 border-blue-600">
            ...
            <AlertDescription>
              You have client-level access. You can create support tickets and view your account information.
            </AlertDescription>
          </Alert>
        </div>
      )}
      ...
      {/* Workspace Overview */}
      {currentWorkspace && hasWorkspaceAccess(currentWorkspace.id, currentWorkspace.requiredAccess) && (
        <div className="px-6 py-4 border-b border-gray-800">
      ...
      <div className="px-6 py-4 border-b border-gray-800">
        ...
        <div className="flex space-x-3">
          {permissions.canCreateTickets && (
            <Button variant="outline" size="sm" className="flex items-center space-x-2 border-gray-600">
...
// Workspace Components with Role-Based Features
function CustomerOperationsWorkspace() {
  const { hasFeatureAccess } = useRoleBasedAccess();

  const availableTabs = [
    { id: "customers", label: "Customer Management", feature: "customers" },
    { id: "support", label: "Support Ticketing", feature: "support" },
    { id: "onboarding", label: "Onboarding & KYC", feature: "onboarding" },
    { id: "accounts", label: "Account Management", feature: "accounts" }
  ].filter(tab => hasFeatureAccess("customer-ops", tab.feature));
      ...
      <Tabs defaultValue={availableTabs[0]?.id} className="w-full">
        ...
        {availableTabs.map(tab => (
          <TabsContent key={tab.id} value={tab.id}>
            {tab.id === "customers" && <CustomerManagement />}
            {tab.id === "support" && <SupportTicketing />}
            {tab.id === "onboarding" && <OnboardingKYCComponent />}
            {tab.id === "accounts" && <AccountManagementComponent />}
          </TabsContent>
        ))}
      </Tabs>
    </div>
