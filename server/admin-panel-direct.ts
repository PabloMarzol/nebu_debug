...
// Direct admin panel implementation to bypass route registration issues
export function setupAdminPanelRoutes(app: Express) {
  console.log('[Admin Panel] Setting up admin panel routes directly...');
  ...
  app.get('/api/admin-panel/test', (req, res) => {
    res.json({ message: 'Admin panel routes are working!', timestamp: new Date().toISOString() });
  });

  console.log('[Admin Panel] All admin panel routes registered successfully');
}